...
function cardealer_get_current_post_id(){
	...
	if( is_archive() ) {
		...
		$page = get_page_by_path( $post_type->rewrite['slug'] ); // get slug
		if( isset( $page-> ID ) && $page-> post_name == 'cars' ){
			$postId = $page-> ID;
		}
	}
	return $postId;
...
function cardealer_wp_body_classes( $classes ) {
	...
	$enable_custom_layout = get_post_meta($postId, 'enable_custom_layout', true);	
	if( $enable_custom_layout == 1 ){
		$page_layout = get_post_meta($postId, 'page_layout', true);
