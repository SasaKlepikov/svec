...
function cdhl_shortcode_promocode_image($atts) {
	...
	<div id="<?php echo esc_attr($element_id);?>" class="<?php echo esc_attr($element_classes);?>">
		<div id="promocode_img_<?php echo esc_attr($element_id);?>" class="promocode_img_container">
			<img class="img-responsive" src="<?php echo esc_url(wp_get_attachment_url($atts['promocode_image'], 'full'));?>" alt="<?php echo !empty($promocode_image_arr['alt'])? $promocode_image_arr['alt']: esc_html__('Promocode Image', 'cardealer-helper')?>">
		</div>
 ...
 function cdhl_promocode_image_shortcode_vc_map() {
	if ( function_exists( 'vc_map' ) ) {
		...
		vc_map( array(
			...
			"description"            => esc_html__( "Potenza Promocode Image( with print functionality) to put on promocode page", 'cardealer-helper'),
