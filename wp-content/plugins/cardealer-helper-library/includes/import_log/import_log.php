...
function cardealer_import_log_menu(){	
	add_submenu_page( 'log-list', esc_html('Import Log','cardealer-helper'), esc_html('Import Log','cardealer-helper'), 'manage_options', 'import-log', 'cdhl_get_import_log_data');
}

function cdhl_get_import_log_data()
{
	// Datatable Style
	wp_register_style( 'import_log_dataTable_css', trailingslashit(CDHL_URL).'/css/export_log/jquery.dataTables.min.css');
	wp_enqueue_style( 'import_log_dataTable_css' );
	
	// Datatable Script
	wp_register_script( 'import_log_dataTable', trailingslashit(CDHL_URL).'js/export_log/jquery.dataTables.min.js', array(),false,true );
	wp_register_script( 'import_log_dataTable_custom', trailingslashit(CDHL_URL).'js/export_log/custom_dataTables.js', array(),false,true );
	...
	<div class="table_area">
		<div class="wrap">
			<div class="msg"></div>
			<h2><?php esc_html_e('Cars Import Log List', 'cardealer-helper');?></h2>
			<table id="import-log" class="display" width="100%" cellspacing="0">
				<thead>
					<tr>
						...
						<th><?php esc_html_e('Status', 'cardealer-helper'); ?></th>
...
function cdhl_search_import_log($term,$start=0,$length=50,$orderby='created',$mode='asc'){
	...
	while ($log_query->have_posts()) : $log_query->the_post();
		...
		$result[$cnt]['post_status'] = get_post_meta(get_the_ID(), 'post_status', true);
