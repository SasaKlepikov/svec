...
class CDHL_Version {
	...
	public static function cdhl_init() { return;
	...
	?>
		...
		<script type="text/javascript">
			jQuery( '.cdhl-update-now' ).click( 'click', function() {
				return confirm("<?php echo esc_js( __( 'It is strongly recommended that you backup your database before proceeding. Are you sure you wish to run the updater now?', 'cardealer-helper' ) ); ?>");
			});
		</script>
		<?php
	}
	...
	public static function cdhl_status() {
		$current_cdhl_version  = get_option( 'cdhl_version', null );
		if ( is_null( $current_cdhl_version ) ) return;
		...
		if ( version_compare( $current_cdhl_version, max( array_keys( self::$db_updates ) ), '<' ) ) { 
			...
			if( !empty($get_version_status) && $get_version_status == 'updated' )
