		...
		switch($tag['value']) {
			...
			case 'table': {
				...
				foreach ($dom[($dom[$key]['parent'])]['trids'] as $j => $trkey) {
					...
					if (isset($prevtrkey) && isset($dom[$trkey]['startpage']) && isset($dom[$prevtrkey]['endpage']) AND ($dom[$trkey]['startpage'] > $dom[$prevtrkey]['endpage'])) {
				...
				foreach ($table_el['trids'] as $j => $trkey) {
					...
					if( isset($parent['cellpos']) && is_array($parent['cellpos']) ) {
					...
					$this->Ln(0, $cell);
					$this->x = (isset($parent['startx']))? $parent['startx'] : '';
