...
function cdhl_make_an_offer_action() { 
	...
	if(isset($_POST['action']) && $_POST['action'] == 'make_an_offer_action') {
        $captcha=$_POST['g-recaptcha-response'];
        $recaptcha_response=cardealer_validate_google_captch($captcha);
		$mao_fname	 	 = sanitize_text_field($_POST['mao_fname']);
		$mao_lname	 	 = sanitize_text_field($_POST['mao_lname']);
		$mao_phone	 	 = sanitize_text_field($_POST['mao_phone']);
		$mao_email		 = sanitize_text_field($_POST['mao_email']);
		$mao_message	 = sanitize_text_field($_POST['mao_message']);
		$mao_reques_price= sanitize_text_field($_POST['mao_reques_price']);
		$car_id 		 = sanitize_text_field($_POST['car_id']);
        
        if (!isset($_POST['mno_nonce'])||!wp_verify_nonce($_POST['mno_nonce'],'make_an_offer')){
			...
			$request_date = (isset($recaptcha_response['challenge_ts']) && !empty($recaptcha_response['challenge_ts']))? $recaptcha_response['challenge_ts'] : '';
				...
				if($post_id) {
					...
					$fromName = empty($car_dealer_options['mao_from_name'])? 'no-reply@'.$_SERVER['HTTP_HOST'] : $car_dealer_options['mao_from_name'];
					$fromMail = empty($car_dealer_options['mao_mail_id_from'])? 'no-reply@'.$_SERVER['HTTP_HOST'] : $car_dealer_options['mao_mail_id_from']; 
					$replyToMail = empty($mao_email)? 'no-reply@'.$_SERVER['HTTP_HOST'] : $mao_email;
					$replyToName = empty($mao_fname)? 'User' : $mao_fname;
					$subject = empty($car_dealer_options['mao_subject'])? esc_html__('Make an Offer Mail', 'cardealer-helperer') : $car_dealer_options['mao_subject'];
    				...
    				if( isset($car_dealer_options['mao_adf_mail_to']) && !empty($car_dealer_options['mao_adf_mail_to']) ) {
						$mail_set = 1;
    					$to = $car_dealer_options['mao_adf_mail_to'];
    					// Mail body
    					$adf_headers = $headers;	
					...
					// Mail body 
					$fields = array(
						'CD_FROM_NAME' 	=> $car_dealer_options['mao_from_name'], 
					...
					if( isset($car_dealer_options['mao_html_mail_to']) && !empty($car_dealer_options['mao_html_mail_to']) ) {
						$mail_set = 1;
						$to = $car_dealer_options['mao_html_mail_to'];
						// Mail body
						...
						if( !wp_mail( $to, $subject, $mmao_html_body,$html_headers) ) $mail_error = 1;
					}
										
					// Sending Text Mail
					if( isset($car_dealer_options['mao_text_mail_to']) && !empty($car_dealer_options['mao_text_mail_to']) ) { 	
						$mail_set = 1;
						$to = $car_dealer_options['mao_text_mail_to'];
						// Mail body
						...
						if( !wp_mail( $to, $subject, $mmao_text_body, $text_headers) ) $mail_error = 1;
					}    				
					...
					if( $mail_set == 0 ) {
						...
						if( isset($car_dealer_options['mmao_html_body']) && !empty($car_dealer_options['mmao_html_body'])){
							...
							$mmao_html_body = esc_html__('One Inquiry Received', 'cardealer-helper');
						if( !wp_mail( $to, $subject, $mmao_html_body) ) $mail_error = 1;
					}
